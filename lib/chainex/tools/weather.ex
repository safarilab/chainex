defmodule Chainex.Tools.Weather do
  ...
  def new do
    Tool.new(
      name: "get_weather",
      description: "Gets current weather information for a specified location including temperature, conditions, humidity, and wind.",
      parameters: %{
        location: %{
          type: :string,
          required: true,
          description: "The city and/or country to get weather for (e.g., 'New York', 'London, UK', 'Tokyo, Japan')"
        },
        units: %{
          type: :string,
          default: "celsius",
  ...
  defp generate_mock_weather(location, units) do
    ...
    base_temp = case units do
      "fahrenheit" -> 32 + rem(location_hash, 60)  # 32-92°F
      "kelvin" -> 273 + rem(location_hash, 40)      # 273-313K  
      _ -> rem(location_hash, 35) - 5               # -5 to 30°C
    end
    ...
    humidity = 30 + rem(location_hash * 2, 50)  # 30-80%
    wind_speed = rem(location_hash * 3, 20)     # 0-19 mph
    
    wind_directions = ["N", "NE", "E", "SE", "S", "SW", "W", "NW"]
    wind_dir = Enum.at(wind_directions, rem(location_hash, length(wind_directions)))
    
    temp_suffix = case units do
      "fahrenheit" -> "°F"
      "kelvin" -> "K"
      _ -> "°C"
    end
