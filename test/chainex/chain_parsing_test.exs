defmodule Chainex.ChainParsingTest do
  ...
  describe "JSON parsing" do
    ...
    test "fails when input is not a string" do
      ...
      |> Chain.transform(fn _input -> %{name: "Alice"} end)  # Transform to map
      |> Chain.parse(:json)
  ...
  describe "struct parsing" do
    ...
    test "parses map into struct" do
      ...
      chain = Chain.new("placeholder")
      |> Chain.transform(fn _input -> input_map end)  # Transform to provide map
      |> Chain.parse(:struct, TestUser)
      ...
      assert result.name == "Bob"
      assert result.age == 25
      assert result.email == nil  # Default value
    end
  ...
  describe "chaining with other steps" do
    test "chains JSON parsing after LLM call" do
      ...
      chain = Chain.new("Generate some data")
    ...
    test "uses parsed data in subsequent transform" do
      ...
      chain = Chain.new(json_string)
      |> Chain.parse(:json)
      |> Chain.transform(fn parsed_data -> "Hello #{parsed_data["name"]}, you are #{parsed_data["age"]} years old" end)
