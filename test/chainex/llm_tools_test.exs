defmodule Chainex.LLMToolsTest do
  ...
  describe "LLM-driven tool calling" do
    ...
    test "Chain with tools option enables tool calling" do
      ...
      chain = Chain.new("What's the weather in Paris and calculate 20 celsius to fahrenheit")
      |> Chain.with_tools([calculator, weather])
      |> Chain.llm(:mock)  # Should default to tool_choice: :auto when tools present
  ...
  describe "Tool format conversion" do
    ...
    test "handles optional parameters correctly" do
      tool = Tool.new(
        ...
        parameters: %{
          ...
          optional_param: %{type: :integer, required: false, description: "Optional", default: 10}
        },
        function: fn _ -> {:ok, "result"} end
      )
    ...
    test "handles enum parameters" do
      tool = Tool.new(
        ...
        description: "Test tool",
        parameters: %{
          units: %{type: :string, enum: ["celsius", "fahrenheit", "kelvin"], description: "Temperature units"}
        },
        function: fn _ -> {:ok, "result"} end
      )
      ...
      assert anthropic_format["input_schema"]["properties"]["units"]["enum"] == ["celsius", "fahrenheit", "kelvin"]
    end
  end
