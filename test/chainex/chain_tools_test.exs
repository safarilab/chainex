defmodule Chainex.Chain.ToolsTest do
  ...
  describe "Chain.tool/3" do
    test "adds tool step to chain" do
      ...
      chain =
        Chain.new("Calculate {{expression}}")
  ...
  describe "tool execution" do
    ...
    test "chained tool calls" do
      ...
      chain =
        ...
        # Transform the number to a string first
    ...
    test "tool mixed with transforms" do
      ...
      chain =
        Chain.new("Calculate and transform")
        |> Chain.with_tools([calculator])
  ...
  describe "complex tool workflows" do
    test "weather and calculation workflow" do
      ...
      chain =
        Chain.new("Weather and math workflow")
        |> Chain.with_tools([calculator, weather])
        ...
        |> Chain.tool(:calculator, expression: "{{input}}")
    ...
    test "text processing pipeline" do
      ...
      chain =
        Chain.new("Text processing pipeline")
      ...
      assert {:ok, %{characters: 11}} = Chain.run(chain)
    end
    ...
    test "conditional tool usage with variables" do
      ...
      chain =
        Chain.new("Conditional tools based on type")
        |> Chain.with_tools([calculator, weather])

      # Test math operation
      math_result =
        Chain.run(
          chain |> Chain.tool(:calculator, expression: "{{expr}}"),
          %{expr: "5 * 5"}
        )
      ...
      weather_result =
        Chain.run(
          chain |> Chain.tool(:get_weather, location: "{{city}}"),
          %{city: "Boston"}
        )
