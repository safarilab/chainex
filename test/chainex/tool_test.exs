defmodule Chainex.ToolTest do
  ...
  describe "validate_definition/1" do
    ...
    test "rejects empty name" do
      ...
      assert {:error, {:invalid_tool, "Tool name cannot be empty"}} =
               Tool.validate_definition(tool)
    end
    ...
    test "rejects invalid function" do
      ...
      assert {:error, {:invalid_tool, "Function must be a 1-arity function"}} =
               Tool.validate_definition(tool)
    end
  ...
  describe "call/2" do
    ...
    test "validates enum constraints" do
      ...
      assert {:error, {:invalid_parameter_value, :status, _}} =
               Tool.call(tool, %{status: "invalid"})
    end
    ...
    test "validates number constraints" do
      ...
      assert {:error, {:invalid_parameter_value, :age, "must be >= 0"}} =
               Tool.call(tool, %{age: -1})

      assert {:error, {:invalid_parameter_value, :age, "must be <= 150"}} =
               Tool.call(tool, %{age: 200})
    end
    ...
    test "validates string pattern constraints" do
      ...
      assert {:error, {:invalid_parameter_value, :email, _}} =
               Tool.call(tool, %{email: "invalid-email"})
    end
    ...
    test "handles function errors gracefully" do
      ...
      assert {:error, {:function_error, "Intentional error"}} =
               Tool.call(tool, %{should_error: true})
    end
...
defmodule Chainex.Tool.RegistryTest do
  ...
  describe "call/3" do
    test "calls tool from registry" do
      ...
      assert {:ok, 15} =
               Registry.call(registry, "math_tool", %{operation: "multiply", a: 3, b: 5})
    end
    ...
    test "lists tool names and retrieves all tools" do
      tools = [
        Tool.new(
          ...
          parameters: %{},
          function: fn _ -> {:ok, "alpha"} end
        ),
        Tool.new(
          name: "beta",
          description: "Beta tool",
          parameters: %{},
          function: fn _ -> {:ok, "beta"} end
        ),
        Tool.new(
          name: "gamma",
          description: "Gamma tool",
          parameters: %{},
          function: fn _ -> {:ok, "gamma"} end
        )
      ]
  ...
  describe "validate_all/1" do
    ...
    test "detects invalid tools in registry" do
      ...
      registry = %Registry{
        tools: %{"valid" => valid_tool, "invalid" => invalid_tool},
        options: %{}
      }
  ...
  describe "integration scenarios" do
    test "complex workflow with multiple tools" do
      ...
      assert {:ok, "EUR 20"} =
               Registry.call(registry, "format_result", %{amount: 20, currency: "EUR"})
    end
