defmodule Chainex.Chain.PromptTest do
  ...
  describe "Chain.prompt/2 with strings" do
    test "replaces template variables" do
      chain = Chain.new("machine learning")
  ...
  describe "Chain.prompt/2 with Prompt structs" do
    test "uses Prompt struct with defaults" do
      prompt_template = Prompt.new(
        "{{greeting}} {{name}}, explain {{topic}}",
        %{greeting: "Hello"}  # Default value
      )
      
      chain = Chain.new("quantum computing")
  ...
  describe "Chain.prompt/2 chaining" do
    test "chains multiple prompt transformations" do
      chain = Chain.new("artificial intelligence")
      ...
      expected = "Mock response for: Create 3 key takeaways from: Mock response for: Summarize this research: Mock response for: Research: artificial intelligence"
    ...
    test "combines prompt with transform steps" do
      ...
      assert {:ok, result} = Chain.run(chain)
      expected = "Mock response for: Expand on this: MOCK RESPONSE FOR: EXPLAIN MACHINE LEARNING BRIEFLY"
      assert result == expected
    end
    
    test "uses variables across chained prompts" do
      context_prompt = Prompt.new(
        "Context: {{context}}\nQuestion: {{input}}",
      ...
      chain = Chain.new("What is recursion?")
      ...
      expected = "Mock response for: Follow up on the advanced explanation: Mock response for: Context: Teaching advanced programming\nQuestion: What is recursion?"
      assert result == expected
    end
  end
